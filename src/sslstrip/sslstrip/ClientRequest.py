...
#        self.uniqueId      = random.randint(0, 10000)
    ...
    def cleanHeaders(self):
        ...
        mappedHeaders = {}
        for k in headers:
            mappedHeaders[k.decode('utf-8')] = headers[k].decode('utf-8')

        if 'accept-encoding' in mappedHeaders:
            del mappedHeaders['accept-encoding']

        if 'if-modified-since' in headers:
            del mappedHeaders['if-modified-since']

        if 'cache-control' in headers:
            del mappedHeaders['cache-control']
